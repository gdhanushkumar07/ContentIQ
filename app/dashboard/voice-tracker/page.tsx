...
import { Activity, TrendingUp, Clock, AlertCircle, Lock } from 'lucide-react'
...
export default function VoiceTrackerPage() {
  return (
    <div className="relative w-full">
      <div className="absolute -inset-4 z-50 flex flex-col items-center pt-[25vh] bg-white/40 dark:bg-[#0A0710]/40 backdrop-blur-[8px] rounded-[32px]">
        <div className="bg-white dark:bg-[#1a1a2e] p-4 rounded-full mb-4 shadow-xl border border-slate-200 dark:border-white/10 flex items-center justify-center">
          <Lock size={32} className="text-slate-600 dark:text-slate-300" strokeWidth={2} />
        </div>
        <h2 className="text-xl font-bold tracking-tight text-slate-800 dark:text-white mb-2">Future Implementation</h2>
        <p className="text-[14px] font-medium text-slate-500 dark:text-[#A1A1AA] text-center max-w-sm px-4">
          We're actively building this feature. It will be unlocked in an upcoming release.
        </p>
      </div>

      <div className="space-y-8 w-full opacity-30 pointer-events-none select-none" aria-hidden="true">
